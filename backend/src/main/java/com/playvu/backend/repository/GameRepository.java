...
public interface GameRepository extends JpaRepository<Game, Integer> {
    ...
    @Query(value = "SELECT g.game_id, g.max_players, g.sub_field_id, g.organizer_id, g.name, g.price, g.start_date AT TIME ZONE 'UTC' AS start_date, g.end_date AT TIME ZONE 'UTC' as end_date, f.picture, f.address AS \"location\", " +
                    ...
                    "AND g.end_date > CURRENT_TIMESTAMP AT TIME ZONE 'UTC'", 
            nativeQuery = true)
    ...
    @Query(value = "SELECT g.game_id, g.organizer_id, g.max_players, g.name, g.start_date AT TIME ZONE 'UTC' AS \"start_date\", g.end_date AT TIME ZONE 'UTC' AS \"end_date\", g.price, mf.picture, mf.address AS \"location\", " +
               "(SELECT COUNT(gp.participant_id) FROM game_participant gp WHERE gp.game_id = g.game_id) AS \"playerCount\", " + 
    ...
    Map<String, Object> findByGameId(@Param("game_id") Integer game_id);
        ...
        @Query(value = "SELECT g.max_players \"maxPlayers\", g.name, g.price, CONCAT(mf.address, ', ', mf.city) AS \"location\", "
                        ...
                        "JOIN users u ON g.organizer_id = u.user_id " +
                        "JOIN field mf ON sf.master_field_id = mf.field_id " +
                        "WHERE g.game_id = :game_id", nativeQuery = true)
        ...
        @Query(value = "SELECT COUNT(*) = 0 FROM game g " +
                        ...
                        "AND NOT (:endDate <= g.start_date OR :startDate >= g.end_date)", nativeQuery = true)
        Boolean checkNoGameConflict(@Param("subFieldId") Integer subFieldId,
        ...
        @Query(value = "SELECT CASE WHEN (SELECT COUNT(*) FROM game_participant gp WHERE gp.game_id = :gameId) >= g.max_players THEN true ELSE false END "
                        +
                        "FROM game g " +
                        "WHERE g.game_id = :gameId", nativeQuery = true)
        Boolean isGameFull(@Param("gameId") Integer gameId);

        @Query(value = "SELECT CASE WHEN COUNT(*) > 0 THEN TRUE ELSE FALSE END " +
                        "FROM game g " +
                        "WHERE g.sub_field_id = :subFieldId AND g.start_date > CURRENT_TIMESTAMP AT TIME ZONE 'UTC'", nativeQuery = true)
